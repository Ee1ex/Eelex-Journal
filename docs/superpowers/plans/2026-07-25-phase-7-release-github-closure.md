...
- [ ] **Step 1: 记录两地访问事实**
  ...
  每条记录必须包含日期、地区、网络环境、浏览器和首页、内容页、`robots.txt`、`sitemap.xml`、`/lab` 的实际结果。缺少任一地区记录时，REQ 保持 `In Progress`。

- [ ] **Step 2: 记录首次发布用量与第 7 天流量快照**

  每次记录 Netlify 数据的查看日期、统计窗口与可见的基础请求/带宽或流量摘要；不得接入第三方脚本代替 Netlify 数据。

- [ ] **Step 3: 对照 REQ 逐项关闭文档**

  所有验收项均有证据后，将 REQ 状态更新为 `Done`，在 PROG 追加 DoD 逐项结果、未完成事项和风险，并更新 PRD 与文档索引。若 7 天观察尚未完成，不得将 Phase 7 标记为完成。
